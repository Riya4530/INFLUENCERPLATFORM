...
export default function InfluencersPage() {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/influencers")
      .then((res) => res.json())
      .then((data) => {
        const list = Array.isArray(data?.influencers)
          ? data.influencers
          : Array.isArray(data)
          ? data
          : [];
  ...
  const filteredInfluencers = list.filter((influencer: any) => {
  ...
  return (
    <main className="max-w-7xl mx-auto px-6 py-20">
      <div className="mb-14">
        <h1 className="text-6xl font-bold mb-4">Explore Influencers</h1>
